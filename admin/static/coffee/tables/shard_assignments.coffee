...
class ShardAssignmentsView extends Backbone.View
    initialize: (data) =>
        @listenTo @model, 'change', =>
            console.log "model change causing a render" #RSI (eta reduce)
            @render()
        if data.collection?
            @collection = data.collection
        @current_vdom_tree = h "div"
        console.log "initialize causing a render" #RSI
        @render()

    set_assignments: (assignments) =>
        @collection = assignments
        @listenTo @collection, 'change', =>
            console.log "collection change causing a render" #RSI (eta-reduce)
            @render()
        console.log "set_assignments causing a render" #RSI
        @render()
